...
export function PrimeIQHome() {
  ...
  return <main className="overflow-hidden bg-[#07111d] text-white">
    ...
    <section id="pricing" className="px-5 py-20 md:px-10"><div className="mx-auto max-w-6xl rounded-3xl border border-amber-300/25 bg-[radial-gradient(circle_at_80%_20%,rgba(245,158,11,0.15),transparent_35%),#0b1621] p-8 md:p-14"><p className="text-xs font-bold uppercase tracking-[0.25em] text-amber-300">Founding Member Offer</p><h2 className="mt-4 max-w-3xl font-display text-4xl font-semibold md:text-5xl">$44.28 for your first 3 months.</h2><p className="mt-5 max-w-xl leading-7 text-slate-300">Then $44.28/month. Cancel anytime.</p><Link href="/register" className="mt-8 inline-flex items-center gap-2 rounded-md bg-amber-300 px-6 py-3 font-bold text-[#171006]">Claim the offer <ArrowRight className="h-4 w-4" /></Link></div></section>
...
function TestimonialCard({ testimonial, featured = false }: { testimonial: { displayName: string; rating: number; reviewText: string; experienceContext: string | null }; featured?: boolean }) { return <blockquote className={`rounded-2xl border bg-[#0b1621] p-7 md:p-9 ${featured ? "border-emerald-400/25" : "border-white/10"}`}><div className="flex gap-1 text-amber-300">{Array.from({ length: testimonial.rating }, (_, index) => <Star key={index} className="h-4 w-4 fill-current" />)}</div><p className="mt-6 whitespace-pre-line text-base leading-8 text-slate-300">“{testimonial.reviewText}”</p><footer className="mt-6 border-t border-white/10 pt-5"><p className="font-bold text-white">— {testimonial.displayName}</p>{testimonial.experienceContext && <p className="mt-1 text-xs text-slate-500">{testimonial.experienceContext}</p>}</footer></blockquote>; }
